export default {
  ...
  auth: {
    ...
    signIn: '登录',
    signingIn: '登录中...',
    signInWithApiKey: '使用 API Key 登录',
    apiKeyTab: 'API Key 登录',
    passwordTab: '账号密码',
    apiKeyLabel: 'API Key',
    apiKeyPlaceholder: '请输入令牌管理生成的 API Key',
    apiKeyRequired: '请输入 API Key',
    apiKeyLoginHint: '令牌管理生成的账号可直接使用 API Key 登录。',
    passwordLoginHint: '普通账户、管理员和 Linux.do 登录请使用账号密码。',
    apiKeyLoginNote: '仅令牌管理生成的账号支持 API Key 登录，账号密码登录仍可作为备选方式。',
    createAccount: '创建账户',
    ...
    accountCreatedWithKeySuccess: '账户创建成功！已自动为你生成首个 API 密钥。',
  ...
  dashboard: {
    ...
    manageExistingKeys: '查看、复制或恢复已有密钥',
    viewUsage: '查看使用记录',
  ...
  keys: {
    ...
    createDisabledForApiKeyLogin: '当前使用 API Key 登录，暂不支持创建新密钥。请改用账号密码登录后再创建。',
    createDisabledForManagedTokenUser: '令牌管理生成的账号不支持在前端创建新密钥。',
    createUnavailableEmptyState: '当前登录方式仅支持查看现有密钥，暂不支持创建新密钥。',
    createUnavailableForManagedTokenUser: '令牌管理生成的账号不提供此入口。',
  ...
  admin: {
    ...
    tokens: {
      title: '令牌管理',
      description: '以独立模块一键创建用户、订阅和 API 密钥',
      createToken: '创建令牌',
      searchPlaceholder: '搜索令牌名称、用户邮箱或 API 密钥',
      noTokensYet: '暂无令牌',
      createFirstToken: '创建第一张令牌后，会自动生成用户、订阅和 API 密钥。',
      createdSuccess: '令牌创建成功',
      deletedSuccess: '令牌删除成功',
      deleteToken: '删除令牌',
      deleteConfirm: "确定要删除令牌 '{name}' 吗？此操作会同时删除用户管理中的对应账号，且无法撤销。",
      failedToLoad: '加载令牌列表失败',
      failedToCreate: '创建令牌失败',
      failedToDelete: '删除令牌失败',
      ...
      form: {
        ...
        customKeyLabel: '自定义 API 密钥',
        customKeyPlaceholder: '留空则自动生成',
        customKeyHint: '如需发放固定令牌，可直接填写；否则系统自动生成。',
        groupLabel: '订阅分组',
        groupPlaceholder: '请选择订阅分组',
        validityDaysLabel: '有效期（天）',
        validityDaysPlaceholder: '请输入有效期天数',
        validityDaysHint: '创建后会自动把订阅绑定到对应用户。',
        notesLabel: '备注',
        notesPlaceholder: '可选，便于记录来源或用途'
      }
    },
    ...
    settings: {
      ...
      tokenManagement: {
        title: '令牌管理显示',
        description: '控制后台菜单与登录页是否展示令牌管理相关入口',
        showEntry: '展示令牌管理入口',
        showEntryHint: '开启后，后台菜单显示“令牌管理”，登录页显示“API Key 登录”选项。'
      },
      site: {
        title: '站点设置',
        description: '自定义站点品牌',
