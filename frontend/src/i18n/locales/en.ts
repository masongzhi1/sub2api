export default {
  ...
  auth: {
    ...
    apiKeyLabel: 'API Key',
    apiKeyPlaceholder: 'Enter the API key generated from token management',
    apiKeyRequired: 'API key is required',
    apiKeyLoginHint: 'Accounts created from token management can sign in directly with an API key.',
    passwordLoginHint: 'Use email/password for regular users, administrators, and Linux.do sign-in.',
    apiKeyLoginNote: 'Only token-management accounts support API key login. Email/password remains available as a fallback.',
    ...
    accountCreatedWithKeySuccess: 'Account created successfully! Your first API key is ready.',
  ...
  dashboard: {
    ...
    manageExistingKeys: 'View, copy, or recover existing keys',
  ...
  keys: {
    ...
    createDisabledForApiKeyLogin: 'You are signed in with an API key, so creating a new key is disabled. Sign in with email and password to create one.',
    createDisabledForManagedTokenUser: 'Accounts created by token management cannot create new keys from the frontend.',
    createUnavailableEmptyState: 'This sign-in method only supports viewing existing keys and cannot create new ones.',
    createUnavailableForManagedTokenUser: 'Accounts created by token management do not provide this entry.',
  ...
  admin: {
    ...
    tokens: {
      ...
      description: 'Provision users, subscriptions, and API keys from an isolated admin module',
      createToken: 'Create Token',
      searchPlaceholder: 'Search token name, user email, or API key',
      noTokensYet: 'No tokens yet',
      createFirstToken: 'Create your first token to automatically provision a user, subscription, and API key.',
      createdSuccess: 'Token created successfully',
      deletedSuccess: 'Token deleted successfully',
      deleteToken: 'Delete Token',
      deleteConfirm: "Are you sure you want to delete token '{name}'? This also removes the linked account from user management and cannot be undone.",
      ...
      form: {
        ...
        namePlaceholder: 'Enter a token name',
        customKeyLabel: 'Custom API Key',
        customKeyPlaceholder: 'Leave blank to auto-generate',
        customKeyHint: 'Provide a fixed key if needed, or leave blank for automatic generation.',
        groupLabel: 'Subscription Group',
        groupPlaceholder: 'Select a subscription group',
        validityDaysLabel: 'Validity (days)',
        validityDaysPlaceholder: 'Enter the validity period in days',
        validityDaysHint: 'The subscription is automatically assigned to the new user.',
        notesLabel: 'Notes',
        notesPlaceholder: 'Optional notes for internal tracking'
      }
    },
    ...
    settings: {
      ...
      tokenManagement: {
        title: 'Token Management Visibility',
        description: 'Control whether token-management entry points appear in admin navigation and on the login page',
        showEntry: 'Show token management entry',
        showEntryHint: 'When enabled, the admin menu shows “Token Management” and the login page shows the “API Key” sign-in option.'
      },
      site: {
        title: 'Site Settings',
